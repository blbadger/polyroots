...
def newton_raphson_map(equation, max_iterations, x_range, y_range, t):
	...
	y, x = np.ogrid[0.7: -0.7: y_range*1j, -0.7: 0.7: x_range*1j]
	z_array = x + y*1j
	# z_array = z_array.astype('singlecomplex')
...
plt.style.use('dark_background')
plt.imshow(newton_raphson_map('x^5-x-1', 45, 2200, 2200, 0), cmap='inferno')
plt.axis('off')
plt.savefig('Newton.png', bbox_inches='tight', pad_inches=0, dpi=490)
# plt.show()
plt.close()
